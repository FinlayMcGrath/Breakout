...
void PlaneMesh::InitBuffers(ID3D11Device* device)
{
	...
	for (j = 0; j<(m_resolution - 1); j++)
	{
		for (i = 0; i<(m_resolution - 1); i++)
		{
			...
			m_vertices[index].texture = XMFLOAT2(u, v);
			m_vertices[index].normal = XMFLOAT3(0.0, 1.0, 0.0);
			indices[index] = index;
			index++;
			...
			m_vertices[index].position = XMFLOAT3(positionX, 0.0f, positionZ);
			m_vertices[index].texture = XMFLOAT2(u + increment, v + increment);
			m_vertices[index].normal = XMFLOAT3(0.0, 1.0, 0.0);
			indices[index] = index;
			index++;
			...
			m_vertices[index].position = XMFLOAT3(positionX, 0.0f, positionZ);
			m_vertices[index].texture = XMFLOAT2(u, v + increment);
			m_vertices[index].normal = XMFLOAT3(0.0, 1.0, 0.0);
			indices[index] = index;
			index++;
			...
			m_vertices[index].position = XMFLOAT3(positionX, 0.0f, positionZ);
			m_vertices[index].texture = XMFLOAT2(u + increment, v);
			m_vertices[index].normal = XMFLOAT3(0.0, 1.0, 0.0);
			indices[index] = index;
			index++;
