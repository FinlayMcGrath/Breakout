...
{
	D3D11_BUFFER_DESC matrixBufferDesc, colourBufferDesc;
	...
	// Setup the description of the dynamic colour constant buffer that is in the vertex shader.
...
void ColourShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, XMFLOAT4 colour)
{
	...
	// Send colour data to vertex shader
	...
	colourPtr->colour.x = colour.x;
	colourPtr->colour.y = colour.y;
	colourPtr->colour.z = colour.z;
	colourPtr->colour.w = colour.w;
	deviceContext->Unmap(m_colourBuffer, 0);
	bufferNumber = 1;

	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_colourBuffer);
}
